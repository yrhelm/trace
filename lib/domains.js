...
// Tracker Radar map, loaded once at startup.
// Shape: { domain: [org, category, fingerprintingScore?] }, score 0-3, omitted at 0.
// Rebuild with `node tools/build-trackers.mjs ./tracker-radar`.
let RADAR = null;

// Radar scores fingerprinting 0-3: "likelihood this domain is fingerprinting
// users". ~47% of domains score >=1, so badging at 1 marks half the web and
// says nothing. We badge at >=2 — measured, not inferred. Note what this
// separates: the score is about scripts probing browser APIs, which is a
// different mechanism from cookie/ID tracking. Criteo scores 1 (it tags you
// with an ID instead — that shows up as the "durable ID" signal); a CDN
// serving a fingerprinting library scores 3. Both readings are correct.
...
// domain (eTLD+1) -> { org, category, fingerprinting, fpScore }
//
// SEED wins on wording (org + category); the fingerprinting score always comes
// from Radar. Keeping them separate matters: most SEED domains are also in
// Radar with a score, so letting SEED answer that question too would blank the
// flag on exactly the biggest trackers (doubleclick scores 3).
export function classify(domain) {
  if (!domain) return { org: "unknown", category: "other", fingerprinting: false, fpScore: 0 };
  const r = RADAR && RADAR[domain];
  const fpScore = (r && r[2]) || 0;
  const fingerprinting = fpScore >= FP_THRESHOLD;
  const seed = SEED[domain];
  if (seed) return { org: seed[0], category: seed[1], fingerprinting, fpScore };
  if (r) return { org: r[0], category: r[1], fingerprinting, fpScore };
  return { org: domain, category: "other", fingerprinting: false, fpScore: 0 };
}
